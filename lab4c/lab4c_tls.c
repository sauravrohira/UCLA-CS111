...
void print_output(float temp)
{
    ...
    char temp_str[15];
    sprintf(temp_str, "%s %.1f\n", curr_time, temp);
    SSL_write(ssl, temp_str, strlen(temp_str));
    if (log_flag == 1)
...
void print_shutdown()
{
    ...
    char curr_time[10];
    strftime(curr_time, sizeof(curr_time), "%H:%M:%S", curr_time_info);
    char shutdown_str[15];
    sprintf(shutdown_str, "%s SHUTDOWN\n", curr_time);
    SSL_write(ssl, shutdown_str, strlen(shutdown_str));
    if (log_flag == 1)
...
{
    char buf[buf_size];
    int read_val = SSL_read(ssl, buf, buf_size);
    if (read_val == -1)
    {
...
int main(int argc, char** argv)
{
    ...
    char id_str[15];
    sprintf(id_str, "ID=%s\n", id_num);
    SSL_write(ssl, id_str, strlen(id_str));
    if(log_flag == 1)
